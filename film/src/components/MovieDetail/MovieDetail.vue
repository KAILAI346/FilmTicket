...
import { formatDate } from '../../common/util/util';
    ...
    export default {
        name: "MovieDetail",
        data(){    return{
            starValue:0,  averageScore:0, commentNum:0, server:'http://localhost:3000',
            jsonData:[],  isShowMovie:false,notWishMovie:true,currentUserCommentDate:[],otherUserCommentDate:[],  
          } 
        },
        created(){
          Indicator.open('Loading...');     this.loadMovieDetail();   
        },
        methods:{         
          async loadMovieDetail(){
            if(this.$route.query.movie_id){
              let json = await getMovieDetail(this.$route.query.movie_id);
              if (json.success_code===200){    this.jsonData = json.data[0];
                //判断电影是否上映
                ...
                if(this.$cookies.get('user_id')){
                  ...
                  if (json.success_code===200){ this.notWishMovie = false; } else{
                ...
                if (commentJson.success_code===200&&commentJson.data.length){
                  let currentIndex=-1,sum=0;     this.commentNum = commentJson.data.length;
                  commentJson.data.forEach((value,index)=>{
                    if (value.user_id==this.$cookies.get('user_id')){ currentIndex = index; }
                  ...
                  this.otherUserCommentDate.sort((a, b)=>{ return b.support_num-a.support_num    
                  });   
                }   }    }    Indicator.close();    },
          //想看按钮处理
          async wishBtnHandle(){
            if (this.$cookies.get('user_id')){  if (this.notWishMovie){
                let json = await wishMovie(this.$cookies.get('user_id'),this.$route.query.movie_id);
                if (json.success_code===200){  this.notWishMovie = false;   this.loadMovieDetail();   } } 
                else { let json = await cancelWishMovie(this.$cookies.get('user_id'),this.$route.query.movie_id);
                if (json.success_code===200){  this.notWishMovie = true; this.loadMovieDetail();   }   }
          ...
          async supportBtnHandle(commentId){ 
            if (this.$cookies.get('user_id')){
              let json = await getCommentById(commentId);   let supportUser,supportNum;
              if (json.success_code===200){
                if (json.data.support_user) {   supportUser = JSON.parse(json.data.support_user);
                  if (supportUser.indexOf(Number(this.$cookies.get('user_id')))>-1) {
                    ...
                    if (!supportUser.length) {   supportUser = undefined;   }
                  } else {  supportUser.push(Number(this.$cookies.get('user_id')));
                    supportNum = supportUser.length; }   } else {
                  supportUser = []; supportUser.push(Number(this.$cookies.get('user_id')));
                  supportNum = supportUser.length;   }
                ...
                if (json.success_code===200){
                  ...
                  if (commentJson.success_code===200&&commentJson.data.length){
                    ...
                    if (this.averageScore!==0&&this.averageScore!==10){
                      this.averageScore = this.averageScore.toFixed(1); }
                    this.starValue = this.averageScore*0.5;
                    if (currentIndex===-1){  this.currentUserCommentDate = [];
                    } else{  this.currentUserCommentDate = commentJson.data.splice(currentIndex,1);  }
                    this.otherUserCommentDate = commentJson.data;
                    this.otherUserCommentDate.sort((a, b)=>{ return b.support_num-a.support_num });   }   }   }
            } else{ this.$router.push('/login');    } 
          },
