...
router.get('/api/getAllUserPassComment', function (req, res) {
  ...
  let sqlStr = 'SELECT * FROM t_user user INNER JOIN t_comment comment ON user.user_id = comment.user_id WHERE comment.movie_id = ? AND (comment.is_pass = 1 OR comment.is_pass = 0) ;';
...
// 删除个人订单
router.post('/api/deleteMyOrderById', function (req, res) {

  let {
    orderId
  } = req.body;
  let sqlStr = 'DELETE FROM t_order WHERE order_id = ? ';
...
//获取当前电影的订单
router.get('/api/admin/ZgetCurrentPageOrder', function (req, res) {
  let {
    currentPage,
    pageSize,
    input
  ...
  let start = Number((currentPage - 1) * pageSize);
  pageSize = Number(pageSize);
  let sqlStr = 'SELECT * FROM t_order INNER JOIN t_schedule ON t_order.schedule_id = t_schedule.schedule_id INNER JOIN t_movie ON t_movie.movie_id = t_schedule.movie_id INNER JOIN t_cinema ON t_cinema.cinema_id = t_schedule.cinema_id INNER JOIN t_user ON t_order.user_id = t_user.user_id WHERE t_cinema.cinema_id = 1 AND t_movie.name LIKE ? ORDER BY order_id;';
  let total;
  conn.query(sqlStr, ["%" + input + "%"], (error, result, field) => {
    if (error) {
      console.log(error);
  ...
  sqlStr = 'SELECT * FROM t_order INNER JOIN t_schedule ON t_order.schedule_id = t_schedule.schedule_id INNER JOIN t_movie ON t_movie.movie_id = t_schedule.movie_id INNER JOIN t_cinema ON t_cinema.cinema_id = t_schedule.cinema_id INNER JOIN t_user ON t_order.user_id = t_user.user_id WHERE t_cinema.cinema_id = 1 AND t_movie.name LIKE ? ORDER BY order_id LIMIT ?,?;';
  conn.query(sqlStr, ["%" + input + "%", start, pageSize], (error, result, field) => {
    if (error) {
      console.log(error);
    } else {
...
//获取特定影院的影厅
router.get('/api/admin/ZgetCurrentPageHall', function (req, res) {
  let {
    currentPage,
    pageSize,
    input
  ...
  let start = Number((currentPage - 1) * pageSize);
  pageSize = Number(pageSize);
  let sqlStr = 'SELECT * FROM t_hall INNER JOIN t_cinema ON t_cinema.cinema_id = t_hall.cinema_id WHERE t_cinema.cinema_id=1 ORDER BY hall_id;';
  let total;
  conn.query(sqlStr, ["%" + input + "%"], (error, result, field) => {
    if (error) {
      console.log(error);
  ...
  sqlStr = 'SELECT * FROM t_hall INNER JOIN t_cinema ON t_hall.cinema_id = 1 WHERE t_cinema.cinema_id=1 AND t_cinema.cinema_name LIKE ? ORDER BY hall_id LIMIT ?,?;';
  conn.query(sqlStr, ["%" + input + "%", start, pageSize], (error, result, field) => {
    if (error) {
      console.log(error);
    } else {
...
//添加特定影院的影厅信息
router.post('/api/admin/ZaddHallInfo', function (req, res) {
  let {
    cinemaId,
    hallName
  } = req.body;
...
//获取当前影院的排片
router.get('/api/admin/ZgetCurrentPageMovieSchedule', function (req, res) {
  let {
    currentPage,
    pageSize,
    input
  ...
  let start = Number((currentPage - 1) * pageSize);
  pageSize = Number(pageSize);
  let sqlStr = 'SELECT * FROM t_schedule INNER JOIN t_movie ON t_schedule.movie_id = t_movie.movie_id INNER JOIN t_cinema ON t_cinema.cinema_id = t_schedule.cinema_id WHERE t_cinema.cinema_id=1 AND t_movie.name LIKE ? ORDER BY schedule_id ';
  let total;
  conn.query(sqlStr, ["%" + input + "%"], (error, result, field) => {
    if (error) {
      console.log(error);
  ...
  sqlStr = 'SELECT * FROM t_schedule INNER JOIN t_movie ON t_schedule.movie_id = t_movie.movie_id INNER JOIN t_cinema ON t_cinema.cinema_id = t_schedule.cinema_id WHERE t_cinema.cinema_id=1 AND t_movie.name LIKE ? ORDER BY schedule_id LIMIT ?,?;';
  conn.query(sqlStr, ["%" + input + "%", start, pageSize], (error, result, field) => {
    if (error) {
      console.log(error);
    } else {
