...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        name: [{ required: true, message: "影厅名不能为空", trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    async loadCurrentPageHall(currentPage, pageSize, input) {
      let json = await ZgetCurrentPageHall(currentPage, pageSize, input);
    ...
    async manageHallInfo() {
      if (this.dialogTitle === "编辑影厅信息") {
        if (this.hallInfo.name) {
          let json = await updateHallInfo(
            this.hallInfo.hall_id,
            this.hallInfo.cinema_id,
            this.hallOldName,
            this.hallInfo.name
          );
          if (json.success_code === 200) {
    ...
    async addHall() {
      ...
      if (json.success_code === 200) {
        this.options = [{ cinema_id: 1, cinema_name: "中影数字影城" }];
        console.log(this.options);
        this.options = JSON.parse(
          JSON.stringify(this.options).replace(/cinema_id/g, "value")
        );
        this.options = JSON.parse(
          JSON.stringify(this.options).replace(/cinema_name/g, "label")
        );
      }
