  ...
  <div id="movie-schedule">
    ...
    <div class="schedule-table">
      <el-table border :data="tableData" style="width: 100%">
        ...
        <el-table-column label="放映大厅" prop="hall_name"> </el-table-column>
        <el-table-column label="放映日期" prop="show_date"> </el-table-column>
        <el-table-column label="放映时间" prop="show_time"> </el-table-column>
      ...
      >
        ...
        >
          <el-form-item label="电影名" prop="movieValue">
            <el-col :span="12">
              <el-select
                v-model="scheduleInfo.movieValue"
                placeholder="请选择"
                @change="changeMovie"
                style="width: 100%"
              >
          ...
          <el-form-item label="影院" prop="cinemaValue">
            <el-col :span="12">
              <el-select
                v-model="scheduleInfo.cinemaValue"
                placeholder="请选择"
                @change="changeCinema"
                style="width: 100%"
              >
          ...
          <el-form-item label="影厅" prop="hallValue">
            <el-col :span="12">
              <el-select
                v-model="scheduleInfo.hallValue"
                placeholder="请选择"
                style="width: 100%"
              >
                <el-option
          ...
          <el-form-item label="放映时间" prop="movieShowTime">
            <el-col :span="12">
              <el-select
                v-model="scheduleInfo.movieShowTime"
                placeholder="请选择时间"
                style="width: 100%"
              >
...
export default {
  ...
  methods: {
    async loadCurrentPageMovieSchedule(currentPage, pageSize, input) {
      let json = await ZgetCurrentPageMovieSchedule(
        currentPage,
        pageSize,
        input
      );
      ...
      MessageBox.confirm(
        ...
        if (value === "confirm") {
          ...
          if (json.success_code === 200) {
            ...
            this.loadCurrentPageMovieSchedule(
              this.currentPage,
              8,
              this.searchInput
            );
    ...
    async addSchedule() {
      ...
      if (json.success_code === 200) {
        this.cinemaOptions = [{ cinema_id: 1, cinema_name: "中影数字影城" }];
        this.cinemaOptions = JSON.parse(
          JSON.stringify(this.cinemaOptions).replace(/cinema_name/g, "label")
        );
        this.cinemaOptions = JSON.parse(
          JSON.stringify(this.cinemaOptions).replace(/cinema_id/g, "value")
        );
      }
    ...
    async addScheduleInfo() {
      if (
        this.scheduleInfo.movieValue &&
        this.scheduleInfo.cinemaValue &&
        this.scheduleInfo.hallValue &&
        this.scheduleInfo.movieShowDate &&
        this.scheduleInfo.movieShowTime &&
        this.scheduleInfo.price &&
        /((^[1-9]\d*)|^0)(\.\d{0,2}){0,1}$/.test(this.scheduleInfo.price)
      ) {
        let json = await addScheduleInfo(
          this.scheduleInfo.movieValue,
          this.scheduleInfo.cinemaValue,
          this.scheduleInfo.hallValue,
          this.scheduleInfo.movieShowDate,
