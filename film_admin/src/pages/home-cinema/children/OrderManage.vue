  ...
  <div id="order-manage">
    ...
    <div class="order-table">
      <el-table border :data="tableData" style="width: 100%">
        ...
        <el-table-column label="票数" align="center" prop="ticket_num">
        </el-table-column>
        <el-table-column
          label="总价"
          width="100"
...
<script>
import { ZgetCurrentPageOrder, deleteOrder } from "../../../api/index";
...
export default {
  ...
  methods: {
    async loadCurrentPageOrder(currentPage, pageSize, input) {
      let json = await ZgetCurrentPageOrder(currentPage, pageSize, input);
    ...
    handleDelete(index, row) {
      MessageBox.confirm(
        "此操作将永久删除该订单所有信息, 是否继续？",
        "提示"
      ).then(async (value) => {
        if (value === "confirm") {
          let json = await deleteOrder(
            row.order_id,
            row.schedule_id,
            row.order_seat_info
          );
